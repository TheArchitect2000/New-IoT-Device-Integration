...
// Use `g++ -std=c++17 install_device.cpp -o output -lcurl` to compile
// Use `sudo ./output` to run the compiled program
...
bool install_device(const std::string& api_url) {
    ...
    std::string json_data = "{\"deviceName\":\"Methane Sensor 1\", \"deviceType\":\"Methane_Sensor\", \"mac\":\"" + macAddress + "\", \"hardwareVersion\":1, \"firmwareVersion\":1, \"parameters\": [{ \"title\": \"ID\", \"UI\": \"text\", \"unit\": 0 }, { \"title\": \"MethaneLevel\", \"UI\": \"text\", \"unit\": \"ppm\" }, { \"title\": \"TempCLevel\", \"UI\": \"text\", \"unit\": \"C\" }, { \"title\": \"TempFLevel\", \"UI\": \"text\", \"unit\": \"F\" }, { \"title\": \"HumidityLevel\", \"UI\": \"text\", \"unit\": \"%\" }, { \"title\": \"BatteryPercentage\", \"UI\": \"text\", \"unit\": \"%\" }], \"isShared\":false}";
    std::string readBuffer;
...
int main() {
    ...
    std::string interface = "eth0"; // On a Raspberry Pi, the primary Ethernet interface is typically "eth0" (for wired) or "wlan0" (for wireless)
    macAddress = getMacAddress(interface);
    
    // Check if MAC address was retrieved successfully
    if (macAddress.empty()) {
    ...
    std::string filePath = "nodes.json";
